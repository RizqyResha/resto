      ...
      <div class="card mt-3">
          ...
          <div class="col-md-12">
              ...
              <ul class="nav nav-tabs mb-3">
	              <li class="nav-item">
		              <a class="nav-link {{ request()->status == '' ? 'active':'' }}" href="?"> Semua </a>
	              </li>
	              <li class="nav-item">
		              <a class="nav-link {{ request()->status == 'SELESAI' ? 'active':'' }}" href="?status=SELESAI"> Selesai </a>
	              </li>
	              <li class="nav-item">
		              <a class="nav-link {{ request()->status == 'BELUM_BAYAR' ? 'active':'' }}" href="?status=BELUM_BAYAR"> Belum Bayar </a>
	              </li>
              </ul>
              <table class="table">
                  <thead>
                  ...
                  <?php $no = 1 ?>
                    @foreach($data as $row)
                      <tr>
                          ...
                          <td>
                            @if($row->status == 'SELESAI')
                              <a href="{{route('adminentriitransaksi.view',['kodeorder'=>$row->kode_order])}}" target='_blank' class="btn btn-primary mr-3"><i class="fas fa-eye"></i></a>
                            @else
